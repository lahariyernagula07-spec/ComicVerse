...
export default function ComicDetail() {
  ...
  const { data: comic, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `${apiBase}/community/comics/${comicId}`
      );
    ...
    enabled: Number.isFinite(comicId),
  });

  const likeMutation = useMutation({
    mutationFn: () =>
      authFetch(
        `${apiBase}/community/comics/${comicId}/like`,
        {
          method: 'POST',
        }
      ).then((r) => r.json()),
        ...
        <div className="animate-pulse space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 mt-8">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className="h-64 bg-cv-card rounded-xl"
              />
            ))}
          </div>
        </div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <Link
        href="/community"
        className="inline-flex items-center gap-1 text-cv-muted hover:text-cv-text text-sm mb-6 transition-colors"
      >
      ...
      <div className="mb-8">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div>
            ...
            <h1
              className="text-4xl font-bold text-cv-text mb-2"
              style={{
            ...
            {comic.description && (
              <p className="text-cv-muted mt-2 max-w-2xl">
                {comic.description}
              </p>
            )}
          ...
          >
            ...
            <Heart
              className={`w-4 h-4 ${
                comic.userHasLiked ? 'fill-current' : ''
              }`}
            />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {(comic.panels ?? []).map(
          (panel: any, i: number) => (
            ...
            <div
              key={panel.id}
              className="bg-cv-card border border-cv-border rounded-xl overflow-hidden panel-border"
            >
              ...
              <div className="p-3 space-y-2">
                ...
                {panel.caption && (
                  <p className="text-xs text-cv-muted italic border-l-2 border-cv-accent pl-2">
                    {panel.caption}
                  </p>
                )}

                {panel.dialogue && (
                  <div className="speech-bubble px-3 py-2 text-sm">
                    {panel.dialogue}
                  </div>
                )}
